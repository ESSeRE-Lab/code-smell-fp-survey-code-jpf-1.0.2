...
/**
 * @version $Id: MockPluginRegistry.java,v 1.3 2007/01/05 13:29:32 ddimon Exp $
 */
public class MockPluginRegistry implements PluginRegistry {
    ...
    private HashMap extensionPoints = new HashMap();
    private HashMap pluginDescriptors = new HashMap();
    private LinkedList pluginFragments = new LinkedList();
    ...
    public ExtensionPoint getExtensionPoint(String pluginId, String pointId) {
        ...
        ExtensionPoint result = (ExtensionPoint) extensionPoints.get(uid);
        if (result == null) {
    ...
    public ExtensionPoint getExtensionPoint(String uniqueId) {
        ExtensionPoint result = (ExtensionPoint) extensionPoints.get(uniqueId);
        if (result == null) {
            throw new IllegalArgumentException(
    ...
    public PluginDescriptor getPluginDescriptor(final String pluginId) {
        PluginDescriptor result =
            (PluginDescriptor) pluginDescriptors.get(pluginId);
        if (result == null) {
            throw new IllegalArgumentException(
